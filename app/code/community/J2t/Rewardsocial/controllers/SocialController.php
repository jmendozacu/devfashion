...
class J2t_Rewardsocial_SocialController extends Mage_Core_Controller_Front_Action
{
    ...
    public function indexAction()
    {
        ...
        $points_default_fb = Mage::getStoreConfig('rewardpoints/socialproduct/facebook_like_points', Mage::app()->getStore()->getId());
        $points_default_gp = Mage::getStoreConfig('rewardpoints/socialproduct/googleplus_points', Mage::app()->getStore()->getId());
        //$points_default_pin = Mage::getStoreConfig('rewardpoints/socialproduct/pinterest_points', Mage::app()->getStore()->getId());
        $points_default_tt = Mage::getStoreConfig('rewardpoints/socialproduct/twitter_points', Mage::app()->getStore()->getId());
        ...
        if ($product_id){
            ...
            if (!$product->getId()){
                ...
                $points_fb = ($product->getRewardsocialFacebook()) ? $product->getRewardsocialFacebook() : $points_default_fb;
                $points_gp = ($product->getRewardsocialGoogle()) ? $product->getRewardsocialGoogle() : $points_default_gp;
                //$points_pin = $product->getRewardsocialPinterest();
                $points_tt = ($product->getRewardsocialTwitter()) ? $product->getRewardsocialTwitter() : $points_default_tt;
            }
        }
        ...
        //recordPoints($pointsInt, $customerId, $orderId, $no_check = true, $link_id = false, $force_date_start = false, $store_id = null, $process_once = false, $max_point_per_customer = 0, $last_entry_gap = 0, $description = null, $object_name = null)
