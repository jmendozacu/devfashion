...
class Rewardpoints_Model_Observer extends Mage_Core_Model_Abstract {
    ...
    protected function appendAdminBlocks(Varien_Event_Observer $observer) {
        if (Mage::getStoreConfig(self::XML_PATH_SHOW_DASHBOARD)){
            $block              = $observer->getBlock();
    ...
    public function addRewardDetailsFront(Varien_Event_Observer $observer){
        ...
        //echo Mage::app()->getRequest()->getControllerName(). ' / '.$block->getNameInLayout() .' >> '.$block->getBlockAlias() .' << '.' >> '.$block->getType() .' << ';
        ...
        if ($block->getType() == 'poll/activePoll' && $poll_points_show && $poll_points){
            $img = '';
            if (Mage::getStoreConfig(self::XML_PATH_DESIGN_SMALL_INLINE_IMAGE_SHOW, Mage::app()->getStore()->getId()) && ($img_size = Mage::getStoreConfig(self::XML_PATH_DESIGN_SMALL_INLINE_IMAGE_SIZE, Mage::app()->getStore()->getId()))){
                $img = '<img src="'.Mage::helper('rewardpoints')->getResizedUrl('j2t_image_small.png', $img_size, $img_size) .'" alt="'.Mage::helper('rewardpoints')->__("Reward points").'" width="'.$img_size.'" height="'.$img_size.'" /> ';
            }
        ...
        if ($block->getNameInLayout() == 'product.tag.list.list.before' && $tag_points_show && $tag_points){
            $img = '';
            if (Mage::getStoreConfig(self::XML_PATH_DESIGN_SMALL_INLINE_IMAGE_SHOW, Mage::app()->getStore()->getId()) && ($img_size = Mage::getStoreConfig(self::XML_PATH_DESIGN_SMALL_INLINE_IMAGE_SIZE, Mage::app()->getStore()->getId()))){
                $img = '<img src="'.Mage::helper('rewardpoints')->getResizedUrl('j2t_image_small.png', $img_size, $img_size) .'" alt="'.Mage::helper('rewardpoints')->__("Reward points").'" width="'.$img_size.'" height="'.$img_size.'" /> ';
            }
        ...
        if ($block->getType() == 'review/helper' && $review_points && $review_points_show
        ...
        if ($active_top_message && ($block->getNameInLayout() == $block_default_top_message || $block->getBlockAlias() == $block_default_top_message)
        ...
        if (is_object(Mage::app()->getFrontController()) && is_object(Mage::app()->getFrontController()->getAction()) && ( 
            ...
            $magento_block = Mage::getSingleton('core/layout');
            if (Mage::app()->getFrontController()->getAction()->getFullActionName() == "sales_order_printInvoice"){
    ...
    public function aggregateRewardpointsData(){
        ...
        if (Mage::getStoreConfig(self::XML_PATH_CRON_REMOVE)){
            ...
            if (version_compare(Mage::getVersion(), '1.4.0', '>=')){
                $collection->getSelect()->where("main_table.rewardpoints_$status_field = ?", 'cancel');
            } else {
                $collection->getSelect()->where("main_table.rewardpoints_state = ?", 'cancel');
            }
 ...
 */
    ...
    public function processAddCallback($observer){
        ...
        if ($object instanceof Mage_Customer_Model_Customer) {
            if ($userId = $object->getRewardpointsReferrer()){
        ...
        if ($object instanceof Mage_Tag_Model_Tag) {
            ...
            if (($pointsInt = Mage::getStoreConfig('rewardpoints/other_points/tag_points', $object->getStoreId())) && ($customer_id = $object->getFirstCustomerId())){
        ...
        if ($object instanceof Mage_Poll_Model_Poll_Vote) {
            ...
            if (Mage::getSingleton('customer/session')){
                if (($pointsInt = Mage::getStoreConfig('rewardpoints/other_points/poll_points', $object->getStoreId())) && (Mage::getSingleton('customer/session')->getCustomer())){
